...
-- 원도우 > 설정 > 연결 > 연결유형 Auto commit by default 체크 해제 확인
...
VALUES('900','홍길동','991215-1234567','hong_gi@or.kr','01011111111',
...
-- 선택 안 된 컬럼은 값이 NULL이 들어감(DEFAULT 존재 시 DEFAULT 설정값으로 삽입됨)
...
SELECT * FROM EMPLOYEE2
WHERE EMP_ID ='900';
-- 조회 다시 해봄(영구 저장 되어있기 때문에 롤백한다 하더라도 되돌리기 안된다)
...
-- EMPLOYEE2 테이블에서 BONUS를 받지 않는 사원의 BOUNS를 0.1로 변경
...
-- 여러 컬럼을 한 번에 수정할 시 콤마로 컬럼을 구분하면됨
-- D9 / 총무부 --> D0 / 전략기획팀
UPDATE DEPARTMENT2
SET DEPT_ID = 'D0',
DEPT_TITLE = '전략기획팀'
...
INSERT INTO EMP_M02
VALUES(999,'곽두원','123456-1234567','KWACK_DW@OR.KR','01012345678',
'D9','J4','S1',9000000,0.5,NULL,SYSDATE,NULL,NULL);

SELECT * FROM EMP_M01; --23명
SELECT * FROM EMP_M02; --5명(기본 4명 + 생성 1명)
...
-- 테이블의 전체 행을 삭제하는 DDL구문 / 테이블을 삭제하는 것이 아니라 전체 행을 삭제
-- DELETE보다 수행속도 더 빠름
...
-- ROLLBACK 후 복구 확인 (복구 안됨을 확인!!)
SELECT * FROM EMPLOYEE3;

-- DELETE : 휴지통에 버리기 (복구 가능)
-- TRUNCFATE : 완전 삭제 (복구 불가능)
