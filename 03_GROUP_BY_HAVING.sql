...
-- EMPLOYEE 테이블에서(FROM 절)
-- 성별(남/여)과(GROUP BY 절)
-- 각 성별 별 인원 수, 급여 합을 (SELECT 절)
-- 인원 수 오름차순으로 조회 (ORDER BY 절)
SELECT COUNT(DECODE(SUBSTR(EMP_NO,8,1),'1','남','2','여'))성별
	   ,COUNT(*)"인원 수"
	   ,SUM(SALARY) "급여 합"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO,8,1),'1','남','2','여') -- 별칭 사용 X -> 아직 SELECT 절 해석 X
ORDER BY "인원 수"; -- 별칭 사용 O -> SELECT 절 해석 완료 후 
...
-- EMPLOYEE 테이블에서 (FROM 절)
-- 부서코드 D5,D6인 부서의 (WHERE 절 -> GROUP BY 절)
-- 부서코드,평균급여,인원 수 조회 (SELECT 절)
SELECT DEPT_CODE 부서코드,ROUND(AVG(SALARY)) 평균급여,COUNT(*) "인원 수" -- 부서코드,평균급여,인원 수 조회
...
-- EMPLOYEE 테이블에서 (FROM 절)
-- 2000년도 이후 입사들의 (WHERE 절)
-- 직급별 급여합을 조회 (GROUP BY 절)
-- 직급코드, 급여 합 조회 (SELECT 절)
...
FROM EMPLOYEE
-- WHERE SALARY >= 3000000 -- 한 사람의 급여가 3백만원 이상이라는 조건(원하는 요구사항 X)
GROUP BY DEPT_CODE
...
UNION ALL
--MINUS
-- 급여가 3백만원 초과인 사원의 사번,이름,부서코드,급여 조회
SELECT EMP_ID,EMP_NAME,DEPT_CODE,SALARY
FROM EMPLOYEE
