...
JOIN TB_CLASS_PROFESSOR USING(CLASS_NO)
JOIN TB_PROFESSOR USING(PROFESSOR_NO)
JOIN TB_DEPARTMENT td ON (C.DEPARTMENT_NO = td.DEPARTMENT_NO)
WHERE CATEGORY = '인문사회';
...
JOIN TB_STUDENT ts ON (td.DEPARTMENT_NO = ts.DEPARTMENT_NO)
JOIN TB_PROFESSOR tp ON(td.DEPARTMENT_NO = tp.DEPARTMENT_NO)
...
FROM TB_STUDENT ts 
JOIN TB_DEPARTMENT td USING (DEPARTMENT_NO)
WHERE td.DEPARTMENT_NAME = (SELECT DEPARTMENT_NAME
                            FROM TB_DEPARTMENT td 
                            JOIN TB_STUDENT ts USING (DEPARTMENT_NO)
                            WHERE STUDENT_NAME = '최경희');
