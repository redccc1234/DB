...
-- 2. 선택 함수 사용
--> DECODE(컬럼명 , 값1, 1인 경우, 값2, 2인 경우, ....,일치하지 않은 경우);
...
-- 2) DEPARTMENT 테이블의 위의 결과가(L1)와 동일한
--   LOCATION_ID와 동일한 DEPT_ID
SELECT DEPT_ID 
FROM DEPARTMENT
...
-- 3) EMPLOYEE 테이블의 위의 결과와 동일한 DEPT_CODE를 가진 사원을 구하시오.
--   사원의 이름, 부서코드 조회
SELECT EMP_NAME,DEPT_CODE
FROM EMPLOYEE
...
-- 3. (단일행) 다중열 서브쿼리
-- 서브쿼리 SELECT절에 나열된 컬럼수가 여러개 일 떄
...
-- 2) 퇴사한 여직원과 같은 부서, 같은 직급인 사원을 조회

-- 방법 1) 단일행 서브쿼리 2개를 이용
...
-- 1. 노옹철과 같은 부서, 같은 직급인 사원 조회(단, 노옹철 제외)
...
(SELECT CEIL(AVG(SALARY))
 FROM EMPLOYEE SUB
 WHERE JOB_CODE = JOB_CODE) 평균
FROM EMPLOYEE MAIN
ORDER BY JOB_CODE;                 

------------------------------------------------------------------------------

-- 7. 인라인 뷰(IINLINE-VIEW)
...
WHERE ROWNUM <= 5; -- 해석순위 2위 WHERE절에서 이제 가상컬럼 1~5행까지만 조회
...
-- EX) 공동 1위가 2명이면 다음 등수는 2위가 아니라 3위로 조회
