 ...
 * 
 * -- 테이블에서 수정할 수 있는 것
 * 1) 제약 조건(추가/삭제) * 제약 조건 자체를 수정하는 구문은 별도로 존재하지 않음
 *                     * 삭제 후 추가해야함
 * 2) 컬럼(추가/수정/삭제)
...
-- *** DEPT_COPY에 DEPT_TITLE 컬럼에 NOT NULL 제약조건 추가/삭제 ***
ALTER TABLE DEPT_COPY
ADD CONSTRAINT DEPT_COPY_TITLE_NN NOT NULL(DEPT_TITLE);
-- ORA-00904: : 부적합한 식별자
--> NOT NULL 제약조건은 새로운 조건을 추가하는 것이 아닌 
--  컬럼 자체에 NULL 허용/비허용을 제거하는 성질 변경의 형태로 인식됨

-- MODIFY(수정하다) 구문을 이용해서 NULL 제거
ALTER TABLE DEPT_COPY
...
-- LNAME 컬럼 추가(기본값 '한국')
ALTER TABLE DEPT_COPY ADD (LANME VARCHAR2(30) DEFAULT '한국')
--> 컬럼이 생성되면서 DEFAULT 값이 자동 삽입되었음을 확인

-- D10 개발 1팀 추가
INSERT INTO DEPT_COPY 
VALUES('D10','개발1팀','L1',DEFAULT,DEFAULT);
-- ORA-12899: "KH_CYA"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
--> DEPT_ID의 데이터 타입이 CAHR(2)이므로 영어+숫자 2글자까지 저장 가능
--> D10은 영어+숫자 3글자!!
--> VARCHAR2(3)으로 변경하기 (남는 바이트 메모리 반환을 위해)
...
ALTER TABLE DEPT_COPY MODIFY LANME DEFAULT 'KOREA';
...
SELECT * FROM DEPT_COPY;
--  ORA-00942: 테이블 또는 뷰가 존재하지 않습니다 ( 테이블명을 변경했기 때문에)
SELECT * FROM DCOPY;
-- 조회 가능!! 
...
-- 부모인 TB1 테이블 삭제하려고 할 떄
DROP TABLE TB1;
-- ORA-02449: 외래 키에 의해 참조되는 고유/기본 키가 테이블에 있습니다
--> 해결 방법
-- 1) 자식, 부모테이블 순서로 삭제
-- 2) ALTER 를 이용해서 FK 제약조건 삭제 후 TB1 삭제 
-- 3) DROP TABLE 식제옵션 CASCADE CONSTRATNTS 사용
--> CASCADE CONSTRATNTS : 삭제하려는 테이블과 연결된 FK 제약조건을 모두 삭제
...
SELECT * FROM TB2;
-- 롤백 안된다. 위에서 DDL구문 중 ALTER를 사용하여 그 시점에 COMMIT 됨
