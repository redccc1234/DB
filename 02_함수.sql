...
-- 함수는 SELECT 문의 
-- SELECT 절, WHERE 절, ORDER BY 절, GROUP BY 절, HANING 절에서 사용 가능

----------------------------- 단일행 함수 -----------------------------

-- LENGTH(컬럼명 | 문자열) : 길이 반환
-- EMPLOYEE 테이블에서
-- 모든 사원의 EMAIL, EMAIL의 길이 조회
...
-- SUBSTR(컬럼명 | 문자열 , 잘라내기 시작할 위치 [,잘라낼 위치]) 
...
-- 주어진 컬럼이나 문자열의 앞,뒤,양 쪽에 있는 지정된 문자를 제거
--> 양 쪽 공백 제거에 많이 사용
...
-- 소수점 위치 미작성 시 소수점 첫 번째 자리에서 반올림
...
-- SYSADATE : 시스템 상의 현재 시간 (년,월,일,시,분,초)을 반환
SELECT SYSDATE FROM DUAL; --2025-10-20 09:14:53.000

-- SYSTIMESTAMP : SYSTIMESTAMP + MS 단위 추가(UTC 정보)
SELECT SYSTIMESTAMP FROM DUAL; --2025-10-20 09:16:48.063 +0900
...
SELECT ADD_MONTHS(SYSDATE,4) FROM DUAL; --2026-02-20 09:30:12.000
SELECT ADD_MONTHS(SYSDATE,-1) FROM DUAL; --2025-09-20 09:30:44.000

-- LAST_DAY(날짜) : 해당 달의 마지막 날짜를 구함
SELECT LAST_DAY(SYSDATE) FROM DUAL;--2025-10-31 09:32:15.000
...
-- 2) 추출한 생년월일을 TO_DATE 타입으로 변경 -> RR패턴을 이용하여 1900년대로 변환
SELECT EMP_NAME,
TO_DATE(SUBSTR(EMP_NO,1,INSTR(EMP_NO,'-')-1) , 'RR MM DD') AS 생년월일
FROM EMPLOYEE; --1962-12-31 00:00:00.000

--3) TO_CHAR 를 이용해서 문자열로 변환 -> 1965년 10월 8일
SELECT EMP_NAME,
TO_CHAR(TO_DATE(SUBSTR(EMP_NO,1,INSTR(EMP_NO,'-')-1) , 'RR MM DD'),'YYYY"년" MM"월" DD"일"') 
AS 생년월일
FROM EMPLOYEE;
...
-- MVL(컬럼명, 컬럼값이 NULL 일 때 바꿀 값) : NULL인 컬럼값을 다른 값으로 변경
...
-- ANG(숫자가 기록된 컬럼명) : 평균
-- 전 직원 급여 평균 조회
SELECT ROUND(AVG(SALARY)) FROM EMPLOYEE; -- 3,047,662
...
-- COUNT() : 행 개수를 헤아려서 반환
-- COUNT(컬럼명) : NULL을 제외한 실제값이 기록된 행 개수를 반환
-- COUNT(*) : NULL을 포함한 전체 행 개수를 반환
-- COUNT(DISTINCT 컬럼명) : 중복을 제거한 행 개수를 반환
