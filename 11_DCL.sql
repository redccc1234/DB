...
-- CREATE USER 사용자명 IDENTIFIED BY 비밀번호;
 CREATE USER cya_sample IDENTIFIED BY sample1234;

-- 2. 새 연결(접속) 추가
-- ORA-01045: 사용자 CYA_SAMPLE는 CREATE SESSION 권한을 가지고있지 않음; 로그온이 거절되었습니다
--> 접속 권한

-- 3. 접속 권한 부여(sys -> 사용자계정)
-- [권한 부여 작성법]
-- GRANT 권한,권한,....TO 사용자명;
GRANT CREATE SESSION TO cya_sample;
...
-- 6. (sys) 테이블 생성 권한 부여와 + TABLESPACE 할당
GRANT CREATE TABLE TO cya_sample;

ALTER USER cya_sample DEFAULT TABLESPACE 
SYSTEM QUOTA UNLIMITED ON SYSTEM;
-- SYSTEM TABLESPACE를 사용자 cya_sample의 
-- 저장소로 무제한 사용 가능하게끔 하는 명령

-- 7. (cya_sample) 테이블 다시 생성
CREATE TABLE TB_TEST(
	PK_COL NUMBER PRIMARY KEY,
	CONTNET VARCHAR2(30)
);
...
GRANT CONNECT, RESOURCE TO cya_sample;
...
GRANT SELECT ON EMPLOYEE TO cya_sample;
...
REVOKE SELECT ON EMPLOYEE FROM cya_sample;
