...
--> DB에 영구반영된 것처럼 보이지만 실제로는 아직 DB에 반영된 것이 아님
-- 트랜잭션에 INSERT 3개 있음
...
-- ROLLBACK 수행된 것 없음. 이미 COMMIT을 통해 반영됨
SELECT * FROM DEPARTMENT2;
...
-- SP2 지점까지 롤백
ROLLBACK TO "SP2";
SELECT * FROM DEPARTMENT2;-- 개발6팀만 삭제(SP2지점 SAVEPOINT 전 개발5팀 있었으므로)

-- SP1 지점까지 롤백
ROLLBACK TO "SP1";
SELECT * FROM DEPARTMENT2;-- 개발5팀 삭제(SP1지점 SAVEPOINT 전 개발4팀 있음)
